...
export function Backlog() {
  const { tasks, loading, addTask, updateTask, deleteTask, reorderTask, moveTask } = useTasks({ status: "backlog" });
  ...
  const handleAdd = useCallback(
    async (input: CreateTaskInput | UpdateTaskInput) => {
      await addTask({ ...(input as CreateTaskInput), status: "backlog" });
  ...
  const handleMoveToPlate = useCallback(
    async (task: Task) => {
      await moveTask(task.id, "plate", 0);
    },
    [moveTask],
  );
    ...
    <div className="p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="px-3 py-1.5 text-sm text-white bg-backlog hover:bg-purple-600 rounded-md"
          aria-label="Add task"
        >
          + Add Task
        </button>
